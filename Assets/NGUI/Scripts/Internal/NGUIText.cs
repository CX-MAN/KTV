...
static public class NGUIText
{
    ...
    static public bool WrapText(string text, out string finalText, bool keepCharCount, bool IsHeadColor = false)
	{
        ...
        if (IsHeadColor)
        {
            if (finalText != "" && finalText.Length > 8)
            {
                ...
                {
                    string colorName = "";
                    colorName = finalText.Substring(0, 8);
                    string[] te = finalText.Split('\n');
                    int count = te.Length;
                    if (count > 1)
                    {
                        finalText = "";
