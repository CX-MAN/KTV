...
public class UITextList : MonoBehaviour
{
    ...
    void OnPress(bool b)
    {
        if (b)
        {
            if (Camera.main != null)
            {
                Vector3 ve3 = UICamera.lastHit.point;
                ve3 = Camera.main.WorldToScreenPoint(ve3);
                ve3.z = 0;
                ve3 = UICamera.currentCamera.ScreenToWorldPoint(ve3);
                string url = textLabel.GetUrlAtPosition(new Vector2(ve3.x, ve3.y));
	...
	protected void Rebuild ()
	{
		if (isValid)
		{
			...
			for (int i = 0; i < mParagraphs.size; ++i)
			{
                ...
                if (NGUIText.WrapText(p.text, out final, false, IsHeadColor))
				{
					p.lines = final.Split('\n');
